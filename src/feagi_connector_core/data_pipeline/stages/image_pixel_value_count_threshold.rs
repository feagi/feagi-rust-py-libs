...
use feagi_connector_core::data_pipeline::stages::{ImagePixelValueCountThresholdStage};
...
#[derive(Clone)]
pub struct PyImagePixelValueCountThresholdStage;

#[pymethods]
impl PyImagePixelValueCountThresholdStage {
    #[new]
    pub fn new(image_properties: PyImageFrameProperties, per_pixel_allowed_range: (u8, u8), acceptable_amount_of_activity_percentage_in_image:(f32, f32)) -> PyResult<(PyImagePixelValueCountThresholdStage, PyPipelineStage)> {
        let per_pixel_allowed_range:RangeInclusive<u8> = per_pixel_allowed_range.0..=per_pixel_allowed_range.1;
        let acceptable_amount_of_activity_in_image: RangeInclusive<WholeImageActivity> = RangeInclusive::new(
        ...
        let result_stage = ImagePixelValueCountThresholdStage::new(image_properties, per_pixel_allowed_range, acceptable_amount_of_activity_in_image);
        if result_stage.is_err() {
            return Err(PyValueError::new_err(format!("{:?}", result_stage.err().unwrap())))
        }
        Ok((PyImagePixelValueCountThresholdStage, PyPipelineStage::new(Box::new(result_stage.unwrap()))))
    }
}

common_stage_implementations!(PyImagePixelValueCountThresholdStage, "ImagePixelValueCountThresholdStage");
