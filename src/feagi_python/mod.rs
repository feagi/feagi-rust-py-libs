...
use feagi_burst_engine::{BurstResult as RustBurstResult, RustNPU as RustNPUCore};
...
impl SynapticPropagationEngine {
    ...
    ///     postsynaptic_potentials: Array of postsynaptic potentials (uint8, 0-255) - FEAGI pstcr_
    ...
    fn build_index(
        ...
        postsynaptic_potentials: PyReadonlyArray1<u8>,
        ...
        let psp = postsynaptic_potentials.as_array();
        let t = types.as_array();
        let v = valid_mask.as_array();

        // Validate array lengths
        let n = source.len();
        if target.len() != n || w.len() != n || psp.len() != n || t.len() != n || v.len() != n {
        ...
        for i in 0..n {
            self.synapses.push(Synapse {
                ...
                conductance: SynapticConductance(psp[i]),  // TODO: Rename field to postsynaptic_potential
...
impl RustNPU {
    ...
    fn new(neuron_capacity: usize, synapse_capacity: usize, fire_ledger_window: usize) -> Self {
        Self {
            npu: Arc::new(Mutex::new(RustNPUCore::new(
                neuron_capacity,
                synapse_capacity,
                fire_ledger_window,
            ))),
    ...
    fn register_cortical_area(&mut self, area_id: u32, cortical_name: String) {
        self.npu
            .lock()
            .unwrap()
            .register_cortical_area(area_id, cortical_name);
    }
            ...
            runner
                ...
                .map_err(|e| {
                    PyErr::new::<pyo3::exceptions::PyIOError, _>(format!(
                        "Failed to attach viz SHM writer: {}",
                        e
                    ))
                })?;
            Ok(())
    ...
    fn attach_motor_shm_writer(&mut self, shm_path: String) -> PyResult<()> {
        if let Some(runner) = &mut self.burst_runner {
            ...
            runner
                ...
                .map_err(|e| {
                    PyErr::new::<pyo3::exceptions::PyIOError, _>(format!(
                        "Failed to attach motor SHM writer: {}",
                        e
                    ))
                })?;
            Ok(())
    ...
    fn register_sensory_agent(
        ...
        if let Some(burst_runner) = &self.burst_runner {
            ...
            sensory_mgr.register_agent(config).map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                    "Failed to register sensory agent: {}",
                    e
                ))
            })?;
            ...
            println!(
                "✅ Registered sensory agent: {} at {} Hz (burst loop running)",
                agent_id, rate_hz
            );
            Ok(())
        } else {
            ...
            println!(
                "⚠️ Burst loop not running - agent {} cannot register yet",
                agent_id
            );
            Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                "Burst loop not running. Load genome to start burst loop, then register agent {}.",
                agent_id
            )))
        }
    }
    ...
    fn deregister_sensory_agent(&mut self, agent_id: String) -> PyResult<()> {
        let burst_runner = self.burst_runner.as_ref().ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>("Burst loop not running")
        })?;
        ...
        sensory_mgr.deregister_agent(&agent_id).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                "Failed to deregister sensory agent: {}",
                e
            ))
        })?;
    ...
    ///     postsynaptic_potentials: List of postsynaptic potentials (0-255) - FEAGI pstcr_
    ...
    fn add_synapses_batch(
        ...
        targets: Vec<u32>,
        weights: Vec<u8>,
        postsynaptic_potentials: Vec<u8>,
        ...
        let psp_vals: Vec<SynapticConductance> =  // TODO: Rename type to SynapticPSP
            postsynaptic_potentials.into_iter().map(SynapticConductance).collect();
        let type_vals: Vec<SynapseType> = synapse_types
            .into_iter()
            .map(|t| {
                if t == 0 {
        ...
        py.allow_threads(|| {
            self.npu.lock().unwrap().add_synapses_batch(
                source_ids,
                target_ids,
                weight_vals,
                psp_vals,
    ...
    fn remove_synapse(&mut self, source: u32, target: u32) -> bool {
        self.npu
            .lock()
            .unwrap()
            .remove_synapse(NeuronId(source), NeuronId(target))
    }
    ...
    fn update_synapse_weight(&mut self, source: u32, target: u32, new_weight: u8) -> bool {
        self.npu.lock().unwrap().update_synapse_weight(
            NeuronId(source),
            NeuronId(target),
            SynapticWeight(new_weight),
        )
    }
    ...
    fn get_neuron_positions_in_cortical_area(
        &self,
        py: Python,
        cortical_area: u32,
    ) -> Vec<(u32, u32, u32, u32)> {
        ...
        py.allow_threads(|| {
            self.npu
                .lock()
                .unwrap()
                .get_neuron_positions_in_cortical_area(cortical_area)
        })
    }
    ...
    fn inject_sensory_batch(&mut self, py: Python, neuron_ids: Vec<u32>, potential: f32) {
        ...
        py.allow_threads(|| {
            self.npu
                .lock()
                .unwrap()
                .inject_sensory_batch(&ids, potential);
        })
    }
    ...
    /// Returns number of neurons updated
    fn update_cortical_area_refractory_period(
        &mut self,
        cortical_area: u32,
        refractory_period: u16,
    ) -> usize {
        self.npu
            .lock()
            .unwrap()
            .update_cortical_area_refractory_period(cortical_area, refractory_period)
    }
    ...
    fn update_cortical_area_leak(&mut self, cortical_area: u32, leak: f32) -> usize {
        self.npu
            .lock()
            .unwrap()
            .update_cortical_area_leak(cortical_area, leak)
    }

    /// Update consecutive fire limit for all neurons in a cortical area
    /// Returns number of neurons updated
    fn update_cortical_area_consecutive_fire_limit(
        &mut self,
        cortical_area: u32,
        limit: u16,
    ) -> usize {
    ...
    /// Returns number of neurons updated
    fn update_cortical_area_snooze_period(
        &mut self,
        cortical_area: u32,
        snooze_period: u16,
    ) -> usize {
        self.npu
            .lock()
            .unwrap()
            .update_cortical_area_snooze_period(cortical_area, snooze_period)
    }
    ...
    fn batch_update_refractory_period(&mut self, neuron_ids: Vec<u32>, values: Vec<u16>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_refractory_period(&neuron_ids, &values)
    }
    ...
    fn batch_update_threshold(&mut self, neuron_ids: Vec<u32>, values: Vec<f32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_threshold(&neuron_ids, &values)
    }
    ...
    fn batch_update_leak_coefficient(&mut self, neuron_ids: Vec<u32>, values: Vec<f32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_leak_coefficient(&neuron_ids, &values)
    }

    /// Batch update consecutive fire limit for multiple neurons
    /// Returns number of neurons updated
    fn batch_update_consecutive_fire_limit(
        &mut self,
        neuron_ids: Vec<u32>,
        values: Vec<u16>,
    ) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_consecutive_fire_limit(&neuron_ids, &values)
    }
    ...
    fn batch_update_snooze_period(&mut self, neuron_ids: Vec<u32>, values: Vec<u16>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_snooze_period(&neuron_ids, &values)
    }
    ...
    fn batch_update_membrane_potential(&mut self, neuron_ids: Vec<u32>, values: Vec<f32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_membrane_potential(&neuron_ids, &values)
    }
    ...
    fn batch_update_resting_potential(&mut self, neuron_ids: Vec<u32>, values: Vec<f32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_resting_potential(&neuron_ids, &values)
    }
    ...
    fn batch_update_excitability(&mut self, neuron_ids: Vec<u32>, values: Vec<f32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_excitability(&neuron_ids, &values)
    }
    ...
    fn batch_update_neuron_type(&mut self, neuron_ids: Vec<u32>, values: Vec<i32>) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_neuron_type(&neuron_ids, &values)
    }

    /// Batch update MP charge accumulation for multiple neurons
    /// Returns number of neurons updated
    fn batch_update_mp_charge_accumulation(
        &mut self,
        neuron_ids: Vec<u32>,
        values: Vec<bool>,
    ) -> usize {
        self.npu
            .lock()
            .unwrap()
            .batch_update_mp_charge_accumulation(&neuron_ids, &values)
    }

    /// Update MP charge accumulation for all neurons in a cortical area
    /// Returns number of neurons updated
    fn update_cortical_area_mp_charge_accumulation(
        &mut self,
        cortical_area: u32,
        mp_charge_accumulation: bool,
    ) -> usize {
        self.npu
            .lock()
            .unwrap()
            .update_cortical_area_mp_charge_accumulation(cortical_area, mp_charge_accumulation)
    }
    ...
    fn get_neuron_state(&self, neuron_id: u32) -> Option<(u16, u16, u16, f32, f32, u16)> {
        self.npu
            .lock()
            .unwrap()
            .get_neuron_state(NeuronId(neuron_id))
    }
    ...
    fn get_fire_ledger_history(
        ...
        self.npu
            .lock()
            .unwrap()
            .get_fire_ledger_history(cortical_idx, lookback_steps)
    }
    ...
    fn get_fire_ledger_window_size(&self, cortical_idx: u32) -> usize {
        self.npu
            .lock()
            .unwrap()
            .get_fire_ledger_window_size(cortical_idx)
    }
    ...
    fn set_fq_sampler_frequency(&mut self, frequency_hz: f64) {
        self.npu
            .lock()
            .unwrap()
            .set_fq_sampler_frequency(frequency_hz);
    }
    ...
    fn set_motor_subscribers(&mut self, has_subscribers: bool) {
        self.npu
            .lock()
            .unwrap()
            .set_motor_subscribers(has_subscribers);
    }
    ...
    fn export_connectome_bytes<'py>(
        ...
        let bytes = bincode::serialize(&snapshot).map_err(|e| {
            PyErr::new::<PyValueError, _>(format!("Failed to serialize connectome: {}", e))
        })?;
    ...
    fn import_connectome_bytes(&mut self, binary_data: &[u8]) -> PyResult<bool> {
        ...
        let snapshot: feagi_connectome_serialization::ConnectomeSnapshot =
            bincode::deserialize(binary_data).map_err(|e| {
                PyErr::new::<PyValueError, _>(format!("Failed to deserialize connectome: {}", e))
            })?;
    ...
    fn apply_projector_morphology(
        ...
        py.allow_threads(|| {
            ...
            .map_err(|e| {
                PyErr::new::<PyValueError, _>(format!("Projector morphology failed: {}", e))
            })
        })
    }
    ...
    fn apply_expander_morphology(
        ...
        py.allow_threads(|| {
            ...
            .map_err(|e| {
                PyErr::new::<PyValueError, _>(format!("Expander morphology failed: {}", e))
            })
        })
    }
    ...
    fn apply_block_connection_morphology(
        ...
        py.allow_threads(|| {
            ...
            .map_err(|e| {
                PyErr::new::<PyValueError, _>(format!("Block connection morphology failed: {}", e))
            })
        })
    }
    ...
    fn apply_patterns_morphology(
        ...
        let rust_patterns: Vec<_> = patterns
            .iter()
            .map(|(src, dst)| {
                let src_pattern = (
...
impl VisualizationEncoder {
    ...
    fn encode(&self, py: Python) -> PyResult<Py<PyBytes>> {
        ...
        let total_size = STRUCT_HEADER_SIZE
            + CORTICAL_COUNT_SIZE
        ...
        for (cortical_id, neurons) in &self.mapped_data.mappings {
            ...
            for i in 0..neuron_count {
                let neuron = neurons.get(i).map_err(|e| {
                ...
                bytes[x_offset..x_offset + 4]
                    .copy_from_slice(&neuron.cortical_coordinate.x.to_le_bytes());
                bytes[y_offset..y_offset + 4]
                    .copy_from_slice(&neuron.cortical_coordinate.y.to_le_bytes());
                bytes[z_offset..z_offset + 4]
                    .copy_from_slice(&neuron.cortical_coordinate.z.to_le_bytes());
...
impl CorticalMappedXYZPNeuronDataDecoder {
    ...
    fn new_from_feagi_byte_structure(byte_structure: &FeagiByteStructure) -> PyResult<Self> {
        ...
        let actual_bytes = if bytes[0] == 2 {
            ...
            if bytes.len() < 8 {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                    "Invalid FeagiByteContainer: too short (len={})",
                    bytes.len()
                )));
            }
        ...
        if struct_type != 11 {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                "Unsupported structure type: {} (expected 11 for NeuronCategoricalXYZP)",
                struct_type
            )));
        }
        ...
        for _ in 0..num_areas {
            ...
            let cid_str = std::str::from_utf8(cid_bytes)
                .unwrap_or("??????")
                .trim_end_matches('\0');
            let cortical_id = CorticalID::from_string(cid_str.to_string()).map_err(|e| {
            ...
            let _data_start = u32::from_le_bytes([
                actual_bytes[offset],
                actual_bytes[offset + 1],
                actual_bytes[offset + 2],
                actual_bytes[offset + 3],
            ]) as usize;
            offset += 4;
...
impl PyPNS {
    ...
    fn new_with_config(
        ...
        zmq_sensory_recv_hwm: Option<i32>,
        ...
        // Configure ZMQ sensory socket options
        #[cfg(feature = "zmq-transport")]
        {
            if let Some(hwm) = zmq_sensory_recv_hwm {
                config.sensory_stream.receive_high_water_mark = hwm;
            }
            if let Some(linger) = zmq_sensory_linger_ms {
                config.sensory_stream.linger_ms = linger;
            }
            if let Some(immediate) = zmq_sensory_immediate {
                config.sensory_stream.immediate = immediate;
            }
    ...
    fn connect_to_burst_engine(&self, rust_npu: &RustNPU) -> PyResult<()> {
        if let Some(burst_runner) = &rust_npu.burst_runner {
            ...
            self.pns
                .lock()
                .unwrap()
                .set_sensory_agent_manager(sensory_mgr);
            Ok(())
        } else {
            Err(PyValueError::new_err(
                "Burst engine not initialized - call start_burst_loop() first",
            ))
        }
    }
    ...
    fn connect_npu_to_sensory_stream(&self, rust_npu: &RustNPU) -> PyResult<()> {
        ...
        self.pns
            .lock()
            .unwrap()
            .connect_npu_to_sensory_stream(npu_arc);
        Ok(())
    }
    ...
    fn set_on_agent_registered(&self, callback: PyObject) -> PyResult<()> {
        ...
        self.pns
            .lock()
            .unwrap()
            .set_on_agent_registered(move |agent_id, agent_type, caps_json| {
                // Invoke Python callback with GIL
                Python::with_gil(|py| {
                    if let Err(e) = callback.call1(py, (agent_id, agent_type, caps_json)) {
                        eprintln!(
                            "🦀 [PNS] Error invoking Python registration callback: {}",
                            e
                        );
                    }
                });
    ...
    fn set_on_agent_deregistered(&self, callback: PyObject) -> PyResult<()> {
        ...
        self.pns
            .lock()
            .unwrap()
            .set_on_agent_deregistered(move |agent_id| {
                // Invoke Python callback with GIL
                Python::with_gil(|py| {
                    if let Err(e) = callback.call1(py, (agent_id,)) {
                        eprintln!(
                            "🦀 [PNS] Error invoking Python deregistration callback: {}",
                            e
                        );
                    }
                });
...
impl PyAgentRegistry {
    ...
    fn new(max_agents: usize, timeout_ms: u64) -> Self {
        Self {
            registry: std::sync::Arc::new(parking_lot::RwLock::new(feagi_pns::AgentRegistry::new(
    ...
    fn get_all_agents_json(&self) -> PyResult<String> {
        ...
        serde_json::to_string(&agents).map_err(|e| {
            PyErr::new::<PyValueError, _>(format!("Failed to serialize agents: {}", e))
        })
    }
    ...
    fn get_agent_json(&self, agent_id: &str) -> PyResult<String> {
        let registry_lock = self.registry.read();
        let agent = registry_lock.get(agent_id).ok_or_else(|| {
            PyErr::new::<PyValueError, _>(format!("Agent not found: {}", agent_id))
        })?;
    ...
    fn register_agent_direct(
        ...
        use feagi_pns::{AgentCapabilities, AgentInfo, AgentTransport, AgentType};
